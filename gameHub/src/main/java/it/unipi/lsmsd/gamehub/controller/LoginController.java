...
public class LoginController {
    ...
    public ResponseEntity<String> registration(@RequestBody RegistrationDTO registrationDTO){
        ...
        if(responseEntity.getStatusCode() != HttpStatus.CREATED){
            log.warn("Registrazione rifiutata per l'utente {}: {}", registrationDTO.getUsername(), responseEntity.getBody());
        ...
        log.error("Creazione utente in Neo4j fallita per {}, rollback dell'utente Mongo {}",
                registrationDTO.getUsername(), responseEntity.getBody());

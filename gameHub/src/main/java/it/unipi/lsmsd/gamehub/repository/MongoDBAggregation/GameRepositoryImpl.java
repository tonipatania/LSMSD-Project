...
public class GameRepositoryImpl implements GameRepositoryCustom {
    ...
    @Override
    public Page<Game> searchGames(String name, List<String> genres, Integer avgScore, Pageable pageable) {
        ...
        if (genres != null && !genres.isEmpty()) {
            for (String genre : genres) {
                ...
                criteriaList.add(Criteria.where("genres").regex("(^|,)\\s*" + Pattern.quote(genre.trim()) + "\\s*(,|$)", "i"));
            }
        }
        if (avgScore != null) {
    ...
    public List<String> findDistinctGenres() {
        // Built as raw pipeline stages (instead of the ProjectionOperation/andExpression DSL) because
        // Spring Data Mongo's string-based aggregation expressions aren't parsed correctly in this project setup.
        AggregationOperation splitGenres = context -> new Document("$project", new Document("genreArray",
                new Document("$split", Arrays.asList(new Document("$ifNull", Arrays.asList("$genres", "")), ","))));

        AggregationOperation unwindGenres = context -> new Document("$unwind", "$genreArray");

        AggregationOperation trimGenre = context -> new Document("$project", new Document("genre",
                new Document("$trim", new Document("input", "$genreArray"))));

        AggregationOperation filterEmpty = context -> new Document("$match", new Document("genre", new Document("$ne", "")));

        AggregationOperation groupGenre = context -> new Document("$group", new Document("_id", "$genre"));

        AggregationOperation sortGenre = context -> new Document("$sort", new Document("_id", 1));

        Aggregation aggregation = newAggregation(splitGenres, unwindGenres, trimGenre, filterEmpty, groupGenre, sortGenre);

        List<Document> results = mongoTemplate.aggregate(aggregation, "games", Document.class).getMappedResults();

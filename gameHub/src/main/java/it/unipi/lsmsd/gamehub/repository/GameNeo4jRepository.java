...
public interface GameNeo4jRepository extends Neo4jRepository<GameNeo4j, String> {
    ...
    // @Query("MATCH (g:GameNeo4j)<-[:ADD]-(u:UserNeo4j) RETURN g.id as id, g.name as name,
    // g.developers as developers, g.categories as categories, g.genres as genres, count(u) as
    // numberOfLinks ORDER BY numberOfLinks DESC LIMIT 10")
    @Query(
            "MATCH (g:GameNeo4j)<-[:ADD]-(u:UserNeo4j) WHERE g.name = $name RETURN count(u) as numberOfLinks")
    ...
    @Query(
            "MATCH (me:UserNeo4j {username: $username})-[:ADD]->(mine:GameNeo4j) "
    ...
    // Fallback per chi ha la wishlist vuota: i giochi piu presenti nelle wishlist della
    // piattaforma.
    @Query(
            "MATCH (g:GameNeo4j)<-[:ADD]-(:UserNeo4j) "
    ...
    @Query(
            "MERGE (a:GameNeo4j {name: $name}) "
                    + "SET a.name = $newName, a.developers = $newDevelopers, "
                    + "a.categories = $newCategories, a.genres = $newGenres")
    void updateGame(
            String name,
            String newName,
